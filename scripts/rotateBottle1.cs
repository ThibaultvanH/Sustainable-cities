...
public class rotateBottle1 : MonoBehaviour
{
    ...
    void Update()
    {
        if (pickup)
        {
            ...
            if (Input.GetKeyDown(KeyCode.E) && trash.GetComponent<opentrash1>().open )
            {
                
                drop = true;
                pickup = false;
                player.GetComponent<bottlecount1>().inhand = false;
                txt.GetComponent<bottlecount1>().count += 1;
            }
    ...
    {
        if (other.tag == "Player" && Rotate && player.GetComponent<bottlecount1>().inhand == false )
        {
            player.GetComponent<bottlecount1>().inhand = true;
            Debug.Log("hit");
            pickup = true;
            Rotate = false;
        }
